...
CREATE TABLE plumbers (
    ...
    -- Source tracking (NEW)
    plumber_source VARCHAR(50) DEFAULT 'manual_registration',
    -- Values: 'manual_registration', 'scraped_checkatrade', 'scraped_mybuilder', 'scraped_trustatrader', 'referral'
    
    can_receive_jobs BOOLEAN DEFAULT true,
    -- false = scraped but not yet registered (cold lead, no payment method)
    -- true = fully registered with payment method (can receive job notifications)
