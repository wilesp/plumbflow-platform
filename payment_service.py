...
class PaymentService:
    ...
    def charge_lead_fee(customer_id: str, amount: int, description: str, metadata: Dict = None) -> Dict:
        ...
        try:
            ...
            elif hasattr(customer, 'default_source') and customer.default_source:
                default_payment_method = customer.default_source
            
            if not default_payment_method:
                raise Exception("No payment method found for customer. Please add a payment method.")
            
            logger.info(f"Using payment method: {default_payment_method} for customer {customer_id}")
            
            # Create payment intent with explicit payment method
            payment_intent = stripe.PaymentIntent.create(
                amount=amount,
                currency='gbp',
                customer=customer_id,
                payment_method=default_payment_method,  # Explicitly specify payment method
            ...
            logger.info(f"Payment result: {payment_intent.status}")
