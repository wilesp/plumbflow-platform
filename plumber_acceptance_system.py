...
class SMSNotificationService:
    ...
    def send_lead_notification(self, plumber_phone: str, plumber_name: str, job_data: Dict, magic_link: str) -> bool:
        ...
        if not self.client:
            print(f"❌ Twilio not configured - would send SMS to {plumber_phone}")
            return False
...
Expires in 2hrs"""
        ...
        try:
            ...
            print(f"✅ SMS sent to {plumber_phone} (SID: {message.sid})")
            return True
...
class LeadAcceptanceService:
    ...
    def accept_lead(self, plumber_id: str, job_id: str, acceptance_method: str) -> Dict:
        ...
        # Get plumber details
        plumber = self.database.get_plumber(plumber_id)
        if not plumber:
            return {
                'success': False,
        ...
        try:
            charge_result = self.payment_service.charge_lead_fee(
                ...
                job_id=job_id,
                amount=lead_fee,
                description=f"Lead: {job.get('jobType', 'Plumbing Job')}"
            )
        ...
        self.database.update_job_status(
            ...
            lead_fee_charged=lead_fee
        )
        
        # Send confirmation SMS to plumber with customer details
        job_details = f"{job.get('jobType', 'Job').replace('_', ' ').title()} - {job.get('postcode', '')}"
        ...
        # Send notification to customer that plumber will call
        # (This would use your email service from earlier)
...
class LeadMatchingService:
    ...
    def find_and_notify_plumbers(self, job_id: str, job_data: Dict, max_plumbers: int = 3):
        ...
        # Find plumbers near job location
        matching_plumbers = self.database.find_plumbers_by_postcode(
        ...
        if not matching_plumbers:
            print(f"❌ No plumbers found for {job_data.get('postcode')}")
            return
        
        # Sort by membership tier (enterprise first)
        tier_priority = {
            'enterprise': 4,
            'professional': 3,
            'starter': 2,
        ...
        matching_plumbers.sort(
            key=lambda p: tier_priority.get(p.get('membership_tier', 'free'), 0),
            reverse=True
        )
        ...
        for idx, plumber in enumerate(matching_plumbers):
            ...
            delay_minutes = self._get_notification_delay(plumber, idx)
            
            # For now, send immediately (in production, use scheduler)
            if delay_minutes == 0:
                self.sms_service.send_lead_notification(
                    plumber_phone=plumber.get('phone'),
                    plumber_name=plumber.get('businessName'),
                    job_data=job_data,
                    magic_link=magic_link
                )
                
                print(f"✅ Notified {plumber.get('businessName')} ({plumber.get('membership_tier', 'free')})")
            else:
                # Schedule for later
                print(f"⏰ Scheduled for {plumber.get('businessName')} in {delay_minutes} mins")
