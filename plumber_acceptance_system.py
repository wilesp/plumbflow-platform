...
class LeadMatchingService:
    ...
    def find_and_notify_plumbers(self, job_id: str, job_data: Dict, max_plumbers: int = 3):
        ...
        print(f"✅ Found {len(matching_plumbers)} plumbers for job {job_id}")
        ...
        for idx, plumber in enumerate(matching_plumbers):
            ...
            if delay_minutes == 0:
                ...
                print(f"✅ Notified {plumber.get('business_name')} ({plumber.get('membership_tier', 'free')})")
            else:
                # Schedule for later
                print(f"⏰ Scheduled for {plumber.get('business_name')} in {delay_minutes} mins")
