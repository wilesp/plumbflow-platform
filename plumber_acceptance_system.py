...
def normalize_phone_for_twilio(phone: str) -> str:
    """
    Normalize UK phone number to E.164 format for Twilio
    
    Twilio requires: +447700900123
    Database has: 07700900123
    
    Args:
        phone: Phone number in any UK format
        
    Returns:
        Phone in E.164 format (+44...)
    """
    ...
    # UK mobile/landline starting with 0
    if clean.startswith('0'):
        return '+44' + clean[1:]  # Replace leading 0 with +44
    
    # Fallback - assume UK number without prefix
    return '+44' + clean
...
class SMSNotificationService:
    ...
    def __init__(self):
        ...
        self.from_name = os.getenv('TWILIO_SENDER_NAME', None)  # Optional: "PlumberFlow"
    ...
    def send_lead_notification(self, plumber_phone: str, plumber_name: str, job_data: Dict, magic_link: str) -> bool:
        ...
        # Normalize phone to E.164 format for Twilio (+44...)
        plumber_phone_e164 = normalize_phone_for_twilio(plumber_phone)
        logger.info(f"📱 Normalized phone: {plumber_phone} → {plumber_phone_e164}")
...
Expires in 2hrs"""
        ...
        try:
            # IMPORTANT: Use phone number for lead notifications (not branded sender)
            # Plumbers need to be able to REPLY "YES" to accept leads
            # Alphanumeric sender IDs don't support replies
            message = self.client.messages.create(
                body=sms_body,
                from_=self.from_number,  # Always use phone number for leads
                to=plumber_phone_e164  # Use E.164 format (+44...)
            )
            
            logger.info(f"✅ SMS sent to {plumber_phone_e164} (SID: {message.sid})")
    ...
    def send_acceptance_confirmation(self, plumber_phone: str, customer_name: str, customer_phone: str, job_details: str) -> bool:
        ...
        Uses branded sender "PlumberFlow" if configured (doesn't need replies)
        """
        
        if not self.client:
            logger.warning(f"✅ Would send acceptance confirmation to {plumber_phone}")
            return False
...
Good luck! 👍"""
        ...
        try:
            # Use branded sender for confirmations (doesn't need replies)
            sender = self.from_name if self.from_name else self.from_number
            
            message = self.client.messages.create(
                body=sms_body,
                from_=sender,
                to=plumber_phone_e164  # Use E.164 format
            )
            
            logger.info(f"✅ Confirmation sent to {plumber_phone} from {sender}")
            return True
...
class LeadAcceptanceService:
    ...
    def accept_lead(self, plumber_id: str, job_id: str, acceptance_method: str) -> Dict:
        ...
        logger.info(f"🔄 Processing lead acceptance: Job {job_id}, Plumber {plumber_id}, Method: {acceptance_method}")
        
        # Get plumber details from database
        logger.info(f"   Fetching plumber details...")
        plumber = self.db.get_plumber(plumber_id)
        if not plumber:
            logger.error(f"   ❌ Plumber not found: {plumber_id}")
            return {
                'success': False,
        ...
        logger.info(f"   Fetching job details...")
        ...
        logger.info(f"   ✅ Job found: {job.get('job_type')}, Status: {job.get('status')}")
        
        # Check if job is still available
        if job.get('status') != 'pending':
            logger.warning(f"   ⚠️  Job already {job.get('status')}")
            return {
                'success': False,
        ...
        logger.info(f"   Calculating lead fee...")
        lead_fee = self._calculate_lead_fee(plumber, job)
        logger.info(f"   ✅ Lead fee: £{lead_fee}")
        
        # Get plumber's Stripe customer ID
        stripe_customer_id = plumber.get('stripe_customer_id')
        logger.info(f"   Stripe customer ID: {stripe_customer_id[:20] if stripe_customer_id else 'MISSING'}...")
        if not stripe_customer_id:
            logger.error(f"   ❌ No payment method on file for plumber {plumber_id}")
            return {
                'success': False,
                'message': 'No payment method on file. Please add a card in your dashboard.'
            }
        
        # Charge plumber's card
        logger.info(f"   💳 Charging £{lead_fee} to Stripe customer...")
        try:
            charge_result = self.payment_service.charge_lead_fee(
                customer_id=stripe_customer_id,  # Use Stripe customer ID
                amount=int(lead_fee * 100),  # Convert £ to pence as INTEGER (Stripe requirement)
                description=f"PlumberFlow Lead: {job.get('job_type', 'Plumbing Job').replace('_', ' ').title()}",
                metadata={
                    'plumber_id': plumber_id,
                    'job_id': job_id,
            ...
            logger.info(f"   ✅ Payment result: {charge_result.get('status')}")
            
            if charge_result.get('status') != 'succeeded':
                logger.error(f"   ❌ Payment failed with status: {charge_result.get('status')}")
                return {
                    'success': False,
                    'message': f"Payment failed: {charge_result.get('status', 'Unknown error')}"
                }
            
            stripe_charge_id = charge_result.get('charge_id')
            logger.info(f"   ✅ Payment successful! Charge ID: {stripe_charge_id}")
        
        except Exception as e:
            logger.error(f"Payment error for plumber {plumber_id}: {str(e)}")
            return {
                'success': False,
        ...
        try:
            ...
            logger.error(f"   ⚠️  Database update failed (but payment succeeded): {db_error}")
        ...
        try:
            ...
            logger.info(f"   ✅ Confirmation SMS sent successfully")
        except Exception as sms_error:
            logger.error(f"   ⚠️  SMS failed (but payment succeeded): {sms_error}")
            # Continue anyway - payment already succeeded
