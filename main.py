...
SUBSCRIPTION_TIERS = {
    "basic": {
        ...
        "interval": "month",
        "radius_miles": 10,
        "price_id": os.getenv("STRIPE_PRICE_BASIC", ""),  # Add via Railway env vars
    ...
    "pro": {
        ...
        "interval": "month",
        "radius_miles": 20,
        "price_id": os.getenv("STRIPE_PRICE_PRO", ""),  # Add via Railway env vars
    ...
    "premium": {
        ...
        "price_id": os.getenv("STRIPE_PRICE_PREMIUM", ""),  # Add via Railway env vars
...
async def dashboard_redirect():
    """Clean URL redirect to dashboard"""
    return RedirectResponse(url="/tradesperson-dashboard.html", status_code=302)
...
# ============================================================================
# SESSION MANAGEMENT - Magic Links & Persistent Sessions
# ============================================================================

@app.post("/api/tradesperson/request-link")
async def request_magic_link(request: Request):
    """
    Request a magic link via SMS
    Tradesperson enters phone number, receives SMS with link
    """
    try:
        # Parse JSON body
        ...
        # Find tradesperson by phone
        cursor.execute("""
            SELECT id, name, trading_name, phone, subscription_status 
            FROM tradespeople 
            WHERE phone = %s AND subscription_status = 'active'
        """, (phone,))
        ...
        if not tradesperson:
            return JSONResponse(
                status_code=404,
                content={"success": False, "error": "Phone number not found or subscription not active"}
            )
        ...
        magic_url = f"https://besttrade.uk/api/magic-link/{magic_token}"
        
        # TEMPORARY: Send without URL to test Twilio
        notification_service.sms_service.send_sms(
            to_phone=tradesperson[3],
            message=f"Best Trade: Your login code is ready. Please check your email for the secure link."
        )
...
@app.get("/api/magic-link/{token}")
async def verify_magic_link(token: str, response: Response):
    """
    Verify magic link and create 30-day session
    Builder clicks link → Gets logged in for 30 days
    """
    try:
        conn = db.get_connection()
        cursor = conn.cursor()
        
        # Find tradesperson by token
        cursor.execute("""
            SELECT id, name, trading_name, magic_link_expires
            FROM tradespeople 
            WHERE magic_link_token = %s
            AND magic_link_expires > %s
            ...
            return HTMLResponse(
                ...
                <html>
                    ...
                    <body style="font-family: Arial; text-align: center; padding: 50px;">
                        ...
                        <p><a href="/tradesperson-dashboard.html">Request a new link</a></p>
                    </body>
                </html>
                """,
                status_code=400
        ...
        tradesperson_id = str(tradesperson[0])
        ...
        response = RedirectResponse(url='/tradesperson-dashboard.html', status_code=302)
        response.set_cookie(
            key="tradesperson_session",
            value=tradesperson_id,
            max_age=30 * 24 * 60 * 60,  # 30 days in seconds
            httponly=True,  # Security: JavaScript can't access
...
@app.get("/api/tradesperson/session")
async def check_session(tradesperson_session: Optional[str] = Cookie(None)):
    """
    Check if tradesperson has valid session
    Dashboard calls this on load → Auto-login if session exists
    """
    if not tradesperson_session:
        return JSONResponse(
            status_code=401,
    ...
    try:
        ...
        cursor.execute("""
            SELECT id, name, trading_name, email, phone, subscription_tier, subscription_status
            FROM tradespeople 
            WHERE id = %s AND subscription_status = 'active'
        """, (tradesperson_session,))
        ...
        return {
            "authenticated": True,
            "tradesperson": {
                "id": str(tradesperson[0]),
                "name": tradesperson[1],
                "trading_name": tradesperson[2],
                "email": tradesperson[3],
                "phone": tradesperson[4],
                "subscription_tier": tradesperson[5],
                "subscription_status": tradesperson[6]
            }
        }
...
@app.post("/api/tradesperson/logout")
async def logout(response: Response):
    """Logout tradesperson - clear session cookie"""
    response.delete_cookie(key="tradesperson_session")
    return {"success": True, "message": "Logged out successfully"}

@app.post("/api/tradesperson/create-session")
async def create_session_after_registration(request: Request, response: Response):
    """
    Create session cookie after successful registration
    Called automatically after payment completes
    """
    try:
        data = await request.json()
        ...
        if not tradesperson_id:
            raise HTTPException(status_code=400, detail="tradesperson_id required")
        ...
        if not cursor.fetchone():
            cursor.close()
            conn.close()
            raise HTTPException(status_code=404, detail="Tradesperson not found or not active")
...
async def post_job(job: CustomerJobRequest):
    ...
    try:
        ...
        cursor.execute("""
            SELECT id, trading_name as business_name, email, phone, subscription_tier, trade_categories
            FROM tradespeople 
            WHERE status = 'active' 
        ...
        for tradesperson in matching_trades:
            ...
            notification_service.email_service.send_email(
                ...
                html_content=f"""
                <h2>New job posted on Best Trade!</h2>
                ...
                <p>Log in to your dashboard to view details and send a quote:<br>
                <a href="https://besttrade.uk/tradesperson-dashboard.html">View Dashboard</a></p>
...
async def register_tradesperson(registration: TradesPersonRegistration):
    ...
    try:
        ...
        cursor.execute("SELECT id FROM tradespeople WHERE email = %s", (registration.email,))
        if cursor.fetchone():
            return JSONResponse(
                status_code=400,
        ...
        cursor.execute("""
            INSERT INTO tradespeople (
                name, trading_name, email, phone, postcode,
                trade_category, trade_categories, subscription_tier,
                subscription_status, status, source, can_receive_jobs,
                registered_at, created_at
            ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            RETURNING id
        """, (
            registration.contact_name,  # name column
            registration.business_name,  # trading_name column
            registration.email,
            registration.phone,
...
async def get_dashboard_data(tradesperson_id: int):
    ...
    try:
        ...
        cursor.execute("""
            SELECT trading_name as business_name, email, subscription_tier, subscription_status,
                   trade_category, trade_categories
            FROM tradespeople WHERE id = %s
        """, (tradesperson_id,))
...
async def create_subscription(request: Request):
    ...
    try:
        ...
        cursor.execute("""
            SELECT email, trading_name as business_name, stripe_customer_id 
            FROM tradespeople WHERE id = %s
        """, (tradesperson_id,))
        ...
        # Ensure price_id is configured
        if not tier_config.get('price_id'):
            raise Exception(
                f"Stripe Price ID not configured for {tier} tier. "
                f"Please create the product in Stripe dashboard and set "
                f"STRIPE_PRICE_{tier.upper()} environment variable in Railway."
            )
        ...
        notification_service.email_service.send_email(
            ...
            html_content=f"""
            <h2>Welcome to Best Trade!</h2>
            
            <p>Your {tier_config['name']} subscription is now active.</p>
            
            <h3>Your benefits:</h3>
            <ul>
            {chr(10).join(f'<li>{feature}</li>' for feature in tier_config['features'])}
            </ul>
            
            <h3>You can now:</h3>
            <ul>
            <li>Browse and quote on jobs in your area</li>
            <li>Receive instant job alerts</li>
            <li>Grow your business with Best Trade</li>
            </ul>
            
            <p>Access your dashboard: <a href="https://besttrade.uk/tradesperson-dashboard.html">View Dashboard</a></p>
...
async def send_quote(quote: QuoteRequest):
    ...
    try:
        ...
        # Verify active subscription
        cursor.execute("""
            SELECT subscription_status, trading_name as business_name, email, phone
            FROM tradespeople 
            WHERE id = %s
        """, (quote.plumber_id,))
        ...
        if job[2]:  # If customer has email
            notification_service.email_service.send_email(
                ...
                html_content=f"""
                <h2>You've received a new quote from {tradesperson[1]}!</h2>
                
                <p><strong>Job:</strong> {job[0]}</p>
                <p><strong>Quote Amount:</strong> £{quote.quote_amount:.2f}</p>
                <p><strong>Estimated Duration:</strong> {quote.estimated_duration or 'Not specified'}</p>
                
                <h3>Message from tradesperson:</h3>
                <p>{quote.message}</p>
                
                <p><strong>Contact:</strong> {tradesperson[2]}<br>
                <strong>Phone:</strong> {tradesperson[3]}</p>
