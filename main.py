...
async def cancel_subscription(request: Request):
    try:
        ...
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute("""
            SELECT t.stripe_subscription_id 
            FROM tradesperson_sessions s 
            JOIN tradespeople t ON s.tradesperson_id = t.id 
            WHERE s.session_token = %s AND s.expires_at > NOW()
        """, (session_token,))
        ...
        if not user or not user.get("stripe_subscription_id"):
            raise HTTPException(status_code=400, detail="No recurring subscription found")
...
async def cancel_featured_ad(request: Request):
    try:
        ...
        cursor.execute("""
            ...
            WHERE tradesperson_id = %s AND trade_category = %s AND status = 'active'
        """, (tradesperson_id, trade_category))
        conn.commit()
        cursor.close()
        conn.close()

        return {"success": True, "message": f"Featured ad for {trade_category} cancelled successfully."}
    except Exception as e:
        print(f"Cancel featured ad error: {e}")
        raise HTTPException(status_code=500, detail=str(e))

# ====================== GET MY FEATURED ADS (DASHBOARD) ======================
@app.get("/api/featured-ads/my-ads")
async def get_my_featured_ads(request: Request):
    try:
        session_token = request.cookies.get("session_token")
        if not session_token:
            raise HTTPException(status_code=401, detail="Not authenticated")
        ...
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute("""
            SELECT f.id, f.trade_category, f.company_name, f.short_description,
                   f.status, f.start_date, f.end_date
            FROM featured_ads f
            JOIN tradesperson_sessions s ON f.tradesperson_id = s.tradesperson_id
            WHERE s.session_token = %s AND s.expires_at > NOW()
        ...
        return {"success": True, "ads": [
            {
                "id": str(ad["id"]),
                "trade_category": ad["trade_category"],
        ...
        print(f"My featured ads error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def stripe_webhook(request: Request):
    ...
    if event["type"] == "customer.subscription.deleted":
        ...
        cursor.execute("""
            UPDATE tradespeople 
            SET subscription_status = 'expired', can_receive_jobs = false
            WHERE stripe_subscription_id = %s
        """, (subscription["id"],))
        conn.commit()
        cursor.close()
        conn.close()
        print(f"Subscription expired for: {subscription['id']}")
...
async def reactivate_subscription(request: Request):
    try:
        ...
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute("""
            SELECT t.id, t.trading_name, t.email, t.stripe_customer_id
            FROM tradesperson_sessions s
            JOIN tradespeople t ON s.tradesperson_id = t.id
            WHERE s.session_token = %s AND s.expires_at > NOW()
        """, (session_token,))
        ...
        if tier == "premium":
            # Cancel any existing subscription so it doesn't fire webhook later
            old_sub_id = user.get("stripe_subscription_id")
            if old_sub_id:
                try:
                    stripe.Subscription.cancel(old_sub_id)
                except Exception:
                    pass  # Already cancelled or doesn't exist, safe to ignore

            # Premium is a one-off PaymentIntent
            payment_intent = stripe.PaymentIntent.create(
                amount=20000,
                currency="gbp",
                customer=customer_id,
                payment_method_types=["card"],
                metadata={"tradesperson_id": tradesperson_id, "tier": "premium"}
            )
            conn = db.get_connection()
            cursor = conn.cursor()
            cursor.execute("""
                UPDATE tradespeople
                SET stripe_customer_id = %s, stripe_subscription_id = NULL,
                    subscription_tier = 'premium', subscription_status = 'active',
                    can_receive_jobs = true
                WHERE id = %s
            """, (customer_id, tradesperson_id))
            conn.commit()
            cursor.close()
            conn.close()
            return {"success": True, "clientSecret": payment_intent.client_secret, "type": "payment_intent"}
        else:
            # Basic/Pro are recurring subscriptions
            subscription = stripe.Subscription.create(
                customer=customer_id,
                items=[{"price": price_id}],
                payment_behavior="default_incomplete",
            ...
            cursor.execute("""
                UPDATE tradespeople
                SET stripe_customer_id = %s, stripe_subscription_id = %s,
                    subscription_tier = %s, subscription_status = 'active',
                    can_receive_jobs = true
                WHERE id = %s
            """, (customer_id, subscription.id, tier, tradesperson_id))
            conn.commit()
            cursor.close()
            conn.close()
            return {
