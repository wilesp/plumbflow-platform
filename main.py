...
def _run_scraper_background(sources, limit_postcodes):
    ...
    try:
        ...
        _scraper_status['total_found'] = scraper.stats['total_found']
        _scraper_status['total_saved'] = scraper.stats['total_saved']
...
async def start_scraper_get():
    """Browser-friendly GET - just visit this URL to start the scraper."""
    ...
    thread = threading.Thread(target=_run_scraper_background, args=(None, None), daemon=True)
    thread.start()
    return JSONResponse({"status": "started", "message": "Scraper started! All 5 sources running.", "check_progress": "/api/scraper/status", "view_results": "/api/scraper/results"})


@app.post('/api/scraper/start')
async def start_scraper(request: Request):
    """
    Start the plumber registry scraper in background.
    
    POST body (optional JSON):
    {
        "sources": ["yell", "watersafe", "ciphe", "aphc", "gas_safe"],  // omit for all
        "limit_postcodes": 10  // omit for full run
    }
    
    Visit https://plumbflow.co.uk/api/scraper/start to begin.
    """
    if _scraper_status['running']:
    ...
    sources = body.get('sources', None)           # None = all 5 sources
    limit_postcodes = body.get('limit_postcodes', None)  # None = all postcodes
    ...
    return JSONResponse({
        'status': 'started',
        'sources': sources or ['yell', 'watersafe', 'ciphe', 'aphc', 'gas_safe'],
        'message': 'Scraper running in background. Check /api/scraper/status for progress.',
...
async def stop_scraper():
    """Hard stop the scraper. Visit https://plumbflow.co.uk/api/scraper/stop"""
...
async def scraper_status():
    ...
    Visit https://plumbflow.co.uk/api/scraper/status
    """
    try:
        db = DatabaseHandler()
...
async def scraper_results(
    ...
    try:
        ...
        if db.conn:
            ...
            rows = list(db.json_data.values())[:limit]
