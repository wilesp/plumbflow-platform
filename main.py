...
async def register_tradesperson(registration: TradesPersonRegistration):
    ...
    try:
        ...
        cursor.execute("SELECT id FROM scraped_plumbers WHERE email = %s", (registration.email,))
        if cursor.fetchone():
            return JSONResponse(
                status_code=400,
...
async def get_dashboard_data(tradesperson_id: int):
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM scraped_plumbers WHERE id = %s
        """, (tradesperson_id,))
