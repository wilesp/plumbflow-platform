...
class Database:
    ...
    def find_plumbers_by_postcode(
        ...
        Uses flexible postcode matching:
        - First tries exact outward code match (e.g., W1A matches W1A)
        - Falls back to postcode area match (e.g., W1 matches W1A, W1B, W1C...)
        - For central London, matches all central postcodes (EC, WC, W, SW, SE, E, N, NW)
        ...
        # Examples: W1A -> W, EC1A -> EC, SW19 -> SW
        postcode_area = ''.join([c for c in postcode_normalized if c.isalpha()])[:2]
        
        # Extract outward code (area + district, e.g., W1, EC1, SW19)
        outward_code = ''
        ...
        # Central London postcodes that should match each other
        central_london = ['EC', 'WC', 'W', 'SW', 'SE', 'E', 'N', 'NW']
        
        # Build flexible matching query
        if postcode_area in central_london:
            # For central London, match all central postcodes (wider reach)
            ...
            # For non-central areas, match postcode area (e.g., HP matches HP1, HP2, HP3...)
            query = """
                SELECT * FROM plumbers
                WHERE status = 'active'
        ...
        logger.info(f"Found {len(result)} plumbers near {postcode} (area: {postcode_area})")
        return result
